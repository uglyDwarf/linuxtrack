...
//    return (int64_t)mach_absolute_time();
  ...
  if (clock_gettime(CLOCK_MONOTONIC, &now))
    return 0;
  return (int64_t)(now.tv_sec * 1000.0 + now.tv_nsec / 1000000.0);
#endif
}
...
bool PS3EYECam::init(uint32_t width, uint32_t height, uint8_t desiredFrameRate)
{
	...
	ov534_reg_write(0xe0, 0x08);
        ...
        const struct timespec tsp[] = {0, 100000000};
#if defined WIN32 || defined _WIN32 || defined WINCE
	Sleep(100);
#else
    nanosleep(tsp, NULL);
...
bool PS3EYECam::open_usb()
{
	...
	//libusb_set_configuration(handle_, 0);
        res = libusb_reset_device(handle_);
        if(res != 0){
                debug("Can't reset device: %d\n", res);
                return false;
        }
        res = libusb_detach_kernel_driver(handle_, 0);
        if(res != 0) {
                debug("can't detach kernel driver: %d\n", res);
        }
